...
def read_certs_file():
    certs = dict()
    with open(".." + os.sep + "certs.txt", "r") as certs_file:
        for line in certs_file:
            line = line.split(":")
            pbkey, prkey = int(line[0]), int(line[0])
            certs.update({pbkey: prkey})
    return certs
...
def handle(sock, conn, prckey, pbckey, pbskey):
    global USER_IS_ADMIN, LOGIN, USER_DIRECTORY, PATH
    USER_IS_ADMIN = LOGIN == ADMIN
    if LOGIN == ADMIN:
        LOGIN += "#"
    shifr.send_encrypted_message(conn, "Вы вошли" + "\n" + LOGIN + "; path: " + pwd()[:-1] + ">>", prckey, pbskey)
    add_log(f"Пользователь {LOGIN} авторизовался")
    while True:
        try:
            request = shifr.get_decrypted_message(conn, pbckey, prckey)
            if request == "exit":
                conn.close()
            response = process(request)
            shifr.send_encrypted_message(conn, response, prckey, pbskey)
            shifr.send_encrypted_message(conn, LOGIN + "; path:" + pwd()[:-1] + ">>", prckey, pbskey)
        except:
            break
    os.chdir(MAIN_DIRECTORY)
    USER_DIRECTORY = MAIN_DIRECTORY
...
def get_password(sock, conn, correctPassword, prckey, pbckey, pbskey):
    shifr.send_encrypted_message(conn, "Введите пароль:", prckey, pbskey)
    password = shifr.get_decrypted_message(conn, pbckey, prckey)
    if password == correctPassword:
        handle(sock, conn, prckey, pbckey, pbskey)
    else:
        get_password(sock, conn, correctPassword, prckey, pbckey, pbskey)


def get_new_password(sock, conn, login, prckey, pbckey, pbskey):
    shifr.send_encrypted_message(conn, "Введите новый пароль:", prckey, pbskey)
    new_password = shifr.get_decrypted_message(conn, pbckey, prckey)
    add_logins(dict([(login, new_password)]), path=os.getcwd())
    handle(sock, conn, prckey, pbckey, pbskey)


def get_login_and_password(sock):
    pbckey = read_key_file("..\\pbc.txt")
    prckey = read_key_file("..\\prc.txt")
    pbskey = read_key_file("..\\pbs.txt")
    conn, (addr, port) = sock.accept()
    global LOGIN
    logins = read_logins(os.getcwd())
    shifr.send_encrypted_message(conn, "Логин:", prckey, pbskey)
    login = shifr.get_decrypted_message(conn, pbckey, prckey)
    create_user_directory(login)
    LOGIN = login
    if login in logins.keys():
        get_password(sock, conn, logins[login], prckey, pbckey, pbskey)
    else:
        get_new_password(sock, conn, login, prckey, pbckey, pbskey)


def get_random_port_from_range(min_port=1000, max_port=1100):
    return random.randint(min_port, max_port)


def get_sock_and_port():
    while True:
        port = get_random_port_from_range()
        try:
            sock = socket.socket()
...
def check_pbckey(conn):
    certs = read_certs_file()
    pbckey = int(conn.recv(1024).decode())
    if pbckey in certs:
        return True
    return False
...
def accept_connection(sock):
    while True:
        try:
            ...
            if check_pbckey(conn):
                new_sock, port = get_sock_and_port()
                conn.send(str(port).encode())
                conn.close()
                get_login_and_password(new_sock)
            else:
                conn.send("Ваш ключ не сертифицирован".encode())
                conn.close()
